...
export default {
  ...
  ethBalance({state, commit, getters}){
    return new Promise(async function(resolve, reject){
      ...
      let l2balance = await util_web3.loadData()
      if(l2balance.error) return resolve();

      commit("ETH_BALANCE", l2balance);
      resolve()
    });
  },
  ...
  connect({state, commit, dispatch}, isInit){
    return new Promise(async function(resolve, reject){
      ...
      // await util_web3.zkConnect(result.coinbase);
  ...
  upf(file){
     ...
     util_web3.uploadImage(file);
  },
  mintn({state,commit,dispatch}, _data){
    console.log('data::::::',_data);
    util_web3.zkMint(_data);
 },
  signLogin({state, commit, dispatch}, payload){
    return new Promise(async function(resolve, reject){
      ...
      // await util_web3.zkConnect(user.coinbase);
