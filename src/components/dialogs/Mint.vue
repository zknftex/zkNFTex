<template>
  <el-dialog :model-value="visible" :show-close="false" :close-on-click-modal="false" :modal="false"
    ...
    <div class="cel-dialog-body">
      ...
      <div class="process">
        <div class="step-info">
          <div class="text" v-if="this.$store.state.isLoading == 0" :key="3">
            <span>{{this.$store.state.loadingMessage}}</span>
          </div>
          <div class="text" v-else-if="this.$store.state.isLoading == 3" :key="4">
            <span>{{this.$store.state.loadingMessage2}}</span>
          </div>
          <div class="text" v-else :key="5">
            <span>{{this.$store.state.loadingMessage3}}</span>
          </div>
          <span v-if="this.$store.state.isLoading == 1" :key="1" :class="step.mint== 2 ? 'finish': ''"
            class="step iconfont icon-seleted"></span>
          <div class="step loading" v-else :key="2">
            <img class="loading-animation" src="@/assets/img/loading.png" />
          </div>
        </div>
        <div class="process-btn">
          ...
          <a href="/items"><el-button @click="$emit('close')"  type="primary" v-if="this.$store.state.isLoading == 1">OK
          </el-button></a>
...
export default {
  data: function () {
    return {
      ...
      loadingMessage: web3.loadingMessage,
      visible: this.show,
      step: {
        approve: 0,
        mint: 0,
  ...
  methods: {
    ...
    async start() {
      ...
      console.log('isLoading::::',web3.isLoading);
      this.onMint().then((res) => {
console.log('isLoading2::::',web3.isLoading);
      });
    ...
    async onMint() {
      ...
      var properties = "";
            if (this.nft.form.properties.length) {
              properties = JSON.stringify(this.nft.form.properties);
            }
      let quantity = 1;
            var _data = {
              ...
              description: this.nft.form.description,
              royalties: JSON.stringify([this.nft.form.royalties]),
              categoryId: this.nft.form.category,
              properties: properties,
              type: asset.type,
              name: this.nft.form.name,
              animUrl:asset.animUrl,
              animStorageId:asset.animStorageId
            };
     let result = this.$store.dispatch("mintn",_data);
